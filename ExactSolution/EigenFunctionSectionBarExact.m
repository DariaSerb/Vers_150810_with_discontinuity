function u = EigenFunctionSectionBarExact(X)
...
B2 = zeros(N_mode, 1);
 ...
 for n_mode = 1:N_mode
     A2(n_mode) = B1*sin(W(n_mode)*xc);
     B2(n_mode) = (S1/S2)*B1*cos(W(n_mode)*xc);
    for n_x = 1:N_x
     u(n_mode, n_x) = u_calc(X(n_x), W(n_mode), A2(n_mode), B2(n_mode));
    end
 end

end

function u = u_calc(x, W, A2, B2)
